  ...
  if(isset($_GET['fetch_rooms']))
  {
    ...
    while($room_data = mysqli_fetch_assoc($room_res))
    {
      ...
      if(!$settings_r['shutdown']){
        ...
        $book_btn = "<button onclick='checkLoginToBook($login,$room_data[id])' class='btn btn-sm w-100 text-white custom-bg shadow-none mb-2'>Забронировать</button>";
      }
        ...
        <div class='card mb-4 border-0 shadow'>
          <div class='row g-0 p-3 align-items-center'>
            ...
            <div class='col-md-2 mt-lg-0 mt-md-0 mt-4 text-center'>
              <h6 class='mb-4'>сом $room_data[price] за ночь</h6>
              $book_btn
              <a href='room_details.php?id=$room_data[id]' class='btn btn-sm w-100 btn-outline-dark shadow-none'>Подробнее</a>
            </div>
          </div>
        </div>
      ";
    ...
    if($count_rooms>0){
      ...
      echo"<h3 class='text-center text-danger'>Нет жилья для показа!</h3>";
    }
