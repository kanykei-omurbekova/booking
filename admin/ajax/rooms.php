<?php 
  ...
  if(isset($_POST['add_room']))
  {
    ...
    if($stmt = mysqli_prepare($con,$q2))
    {
      ...
      $flag = 0;
      die('Запрос не может быть подготовлен - вставка');
    }
  ...
  if(isset($_POST['get_all_rooms']))
  {
    ...
    {
      if($row['status'] == 1){
        $status = "<button onclick='toggle_status($row[id],0)' class='btn btn-dark btn-sm shadow-none'>Активен</button>";
      }
      else{
        $status = "<button onclick='toggle_status($row[id],1)' class='btn btn-warning btn-sm shadow-none'>Неактивен</button>";
      }
